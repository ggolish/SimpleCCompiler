...
void fun_return();
void fun_read(char *f, char *varlist);
void fun_write(char *f, int n);
void print_exit();
void print_header();
void print_footer();
void print_label(char *label);
void print_num_label(char *label, int n);
void push(char *r);
void pop(char *r);
void push_add();
...
statement: TOK_TYPE varlist TOK_SMCO                                    { var_init($1, $2);                                 }
         ...
         | TOK_WRTE TOK_OPPN arglist TOK_CLPN TOK_SMCO                  { fun_write($1, $3);                                }
         | TOK_READ TOK_OPPN varlist TOK_CLPN TOK_SMCO                  { fun_read($1, $3);                                 }
         | TOK_IFCD TOK_OPPN conditional TOK_CLPN iftok statement endif 
         | tokwhlp TOK_OPPN conditional TOK_CLPN dowhlp statement endwh
         | block
         ;

tokwhlp: TOK_WHLP                                                       { startwhile();                                     }
       ;
...
endif:                                                                  { print_num_label("label", stack_pop());            }
     ;
...
void print_header()
{
    ...
    printf("%sextern writeint\n", INDENT);
    printf("%sextern writebin\n", INDENT);
    printf("%sextern writehex\n", INDENT);
    printf("%sextern readint\n", INDENT);
    printf("%sextern readbin\n", INDENT);
    printf("%sextern readhex\n", INDENT);
    print_label("_start");
    printf("%scall main\n", INDENT);
}
...
void push_cond(char *jmp)
{
    ...
    push_num(0);
    stackptr--;
    printf("%sjmp .false%02d\n", INDENT, labelptr);
    print_num_label(".true", labelptr);
    push_num(1);
    print_num_label(".false", labelptr);
}
...
void endwhile()
{
    ...
    t2 = stack_pop();
    t1 = stack_pop();
    printf("%sjmp label%02d\n", INDENT, t1);
    print_num_label("label", t2);
}

void startwhile()
{
    stack_push(labelptr);
    print_num_label("label", labelptr++);
}
